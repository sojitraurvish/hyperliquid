...
const PresetCard = ({
  ...
  <button
    onClick={onClick}
    className={`
      flex items-center gap-3 w-full px-3 py-2.5 rounded-lg transition-all cursor-pointer
      ${isActive
        ? "bg-gray-800 border border-gray-600 ring-1 ring-green-500/30"
  ...
  >
    ...
    {isActive && (
      <svg className="w-4 h-4 text-green-400 ml-auto shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const ThemePreview = ({ upColor, downColor }: { upColor: string; downColor: string }) => (
  <div className="rounded-lg bg-gray-900/80 border border-gray-800 p-3 space-y-2">
    ...
    <div className="flex items-center gap-2">
      {/* Buy button preview */}
      <div
        className="flex-1 h-8 rounded-md flex items-center justify-center text-xs font-medium text-white"
    ...
    <div className="flex items-center gap-3 text-xs">
      ...
      <span className="text-gray-500 ml-auto">Sample</span>
    </div>
  </div>
);
    ...
    >
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button
          ...
          className="h-9 px-4 text-sm rounded-md bg-gray-800 text-gray-300 hover:bg-gray-700 transition-colors cursor-pointer"
        >
          Cancel
        </button>
        <button
          onClick={handleSave}
          className="h-9 px-4 text-sm rounded-md bg-green-500 text-white hover:bg-green-600 transition-colors font-medium cursor-pointer"
        >
          Save
        </button>
      </div>
...
export const ThemePickerButton = ({ onClick }: { onClick: () => void }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 px-2 py-2 text-gray-300 hover:bg-gray-800 rounded-lg transition-all cursor-pointer"
      title="Theme Colors"
    >
